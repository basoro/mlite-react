...
const Pendaftaran: React.FC = () => {
  ...
  const { data: scheduleData, isLoading: isRalanLoading } = useQuery({
    queryKey: ['rawatJalan', date.from, date.to],
  ...
  const { data: igdScheduleData, isLoading: isIgdLoading } = useQuery({
    queryKey: ['igd', date.from, date.to],
    queryFn: () => getIgdList(
      format(date.from, 'yyyy-MM-dd'), 
      format(date.to, 'yyyy-MM-dd'), 
      0, 100
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const data = {
        ...formData,
        tgl_registrasi: format(new Date(), 'yyyy-MM-dd'),
          ...
          <CardContent className="p-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <button
                ...
                className={`w-full text-left px-4 py-2 rounded-md text-sm border transition-colors ${
                  format(date.from, 'yyyy-MM-dd') === format(new Date(new Date().setDate(new Date().getDate() - 1)), 'yyyy-MM-dd') &&
                  format(date.to, 'yyyy-MM-dd') === format(new Date(new Date().setDate(new Date().getDate() - 1)), 'yyyy-MM-dd')
                    ? 'border-emerald-500 bg-white text-emerald-600 font-medium'
                    : 'border-gray-200 hover:bg-gray-50 text-gray-600'
                }`}
              >
            ...
            <h3 className="font-bold mb-4">Statistik Periode Ini</h3>
          ...
          <CardContent className="p-6">
            <div className="mb-6">
              <h2 className="text-xl font-bold flex items-center gap-2">
                ...
                Pendaftaran {format(date.from, 'dd MMM yyyy', { locale: id })}
                {date.to && format(date.to, 'yyyy-MM-dd') !== format(date.from, 'yyyy-MM-dd') && ` - ${format(date.to, 'dd MMM yyyy', { locale: id })}`}
              </h2>
              <p className="text-sm text-gray-500 mt-1">
                Daftar appointment dan pendaftaran untuk periode yang dipilih
              </p>
            </div>
