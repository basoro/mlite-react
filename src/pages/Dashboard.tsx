...
import { getRawatJalanList, getIgdList, getGudangBarangList, getInventoryList } from '@/lib/api';
...
const Dashboard: React.FC = () => {
  ...
  const { data: gudangData } = useQuery({
    queryKey: ['gudangBarang'],
    queryFn: () => getGudangBarangList(1, 100),
  });

  const { data: inventoryData } = useQuery({
    queryKey: ['inventory'],
    queryFn: () => getInventoryList(1, 1000), // Increase limit to catch more items
  });

  // Process Low Stock (Gudang Barang)
  // Need to filter active items. GudangBarang usually doesn't have status.
  // We can map from inventoryData if available.
  const inventoryMap = new Map();
  if (inventoryData?.data) {
    inventoryData.data.forEach((item: any) => {
        inventoryMap.set(item.kode_brng, item);
    });
  }

  const lowStockItems = (gudangData?.data || [])
    .filter((item: any) => {
      // Check status from inventory map
      const dataBarang = inventoryMap.get(item.kode_brng);
      if (dataBarang && dataBarang.status !== '1') return false; // Skip inactive

      const stok = parseFloat(item.stok);
      // Use stokminimal from gudang item first, then databarang
      let stokMin = 10;
      if (item.stokminimal && parseFloat(item.stokminimal) > 0) {
          stokMin = parseFloat(item.stokminimal);
      } else if (dataBarang && dataBarang.stokminimal) {
    ...
    .map((item: any) => {
         const dataBarang = inventoryMap.get(item.kode_brng);
         const minStok = item.stokminimal && parseFloat(item.stokminimal) > 0 ? item.stokminimal : (dataBarang?.stokminimal || 10);
         return `${item.nama_brng} (${item.nm_bangsal || 'Gudang'}) — ${item.stok} / min ${minStok}`;
    });

  // Process Expiring Items (Data Barang)
  const expiringItems = (inventoryData?.data || [])
    .filter((item: any) => {
      ...
      // Show expiring in < 30 days (including expired)
      return days < 30;
    })
    .slice(0, 5) // Limit to 5
    .map((item: any) => {
      const days = differenceInDays(parseISO(item.expire), new Date());
      return `${item.nama_brng} — kadaluarsa ${item.expire} (sisa ${days} hari)`;
    });
          ...
          <div className="bg-card rounded-xl border border-border p-6">
            ...
            <div className="space-y-3">
              ...
              {expiringItems.length > 0 ? (
                ...
                <div className="text-sm text-muted-foreground">Tidak ada obat hampir kadaluarsa</div>
              )}
            </div>
          </div>
        </div>
