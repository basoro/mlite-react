...
import { getKasirRalanList, getStockMovementList, getGudangBarangList, getDiagnosaPasien, GudangBarang, RiwayatBarang } from "@/lib/api";
...
export default function Laporan() {
  ...
  const handlePeriodChange = (value: string) => {
    ...
    switch (value) {
      case 'hari_ini':
        ...
        break;
      case '7hari':
        from = subMonths(today, 0); // Need subDays actually, but let's fix imports first
  ...
  const stats = useMemo(() => {
    ...
    const visitTrend = Object.keys(visitsByDate).map(date => {
      const dailyRevenue = rawatJalanList
        .filter((item: any) => item.tgl_registrasi === date)
        .reduce((sum: number, item: any) => sum + (parseFloat(item.total_tagihan) || 0), 0);
    ...
    // Calculate Total Revenue from the whole period
    const totalRevenue = rawatJalanList.reduce((sum: number, item: any) => sum + (parseFloat(item.total_tagihan) || 0), 0);
    ...
    const salesByName = salesItems.reduce((acc: any, curr: any) => {
      const name = curr.nama_brng || curr.kode_brng;
      if (!acc[name]) {
        acc[name] = { name, sold: 0, revenue: 0 };
      }
      acc[name].sold += parseFloat(curr.keluar);
      // Calculate revenue based on h_beli * quantity
      acc[name].revenue += parseFloat(curr.keluar) * (parseFloat(curr.h_beli) || 0);
      return acc;
    }, {});
    ...
    const kpiData = [
      {
        ...
        note: "Pasien dalam periode ini",
        icon: Users,
        color: "text-blue-500"
      },
      {
        title: "Total Pendapatan",
        value: new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR" }).format(totalRevenue || 0),
      ...
      },
      {
        ...
        value: new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR" }).format(totalInventoryValue),
        note: "Total nilai aset obat",
        icon: TrendingUp,
  ...
  const { data: diagnosaData } = useQuery({
    queryKey: ['diagnosa-pasien', date.from, date.to],
    queryFn: () => getDiagnosaPasien(
  ...
  const topDiseases = useMemo(() => {
    if (!diagnosaData?.data || diagnosaData.data.length === 0) {
      return [{ name: "Belum ada data diagnosa", cases: 0, percentage: 0 }];
    }
    
    const total = diagnosaData.data.reduce((acc: number, curr: any) => acc + parseInt(curr.jumlah), 0);
    
    return diagnosaData.data.map((item: any) => ({
      name: item.nm_penyakit,
      cases: parseInt(item.jumlah),
      percentage: total > 0 ? Math.round((parseInt(item.jumlah) / total) * 100) : 0
    }));
  }, [diagnosaData]);
  ...
  const handleDownload = (type: string) => {
    ...
    let content = "";
    let filename = `laporan-${type}-${format(new Date(), 'yyyy-MM-dd')}.csv`;

    if (type === 'kunjungan') {
      content = "Tanggal,Jumlah Kunjungan,Pendapatan\n";
      stats.visitTrend.forEach(item => {
        content += `${item.date},${item.count},${item.revenue}\n`;
      });
    } else if (type === 'resep') {
      content = "Nama Obat,Terjual\n";
      stats.medicineSales.forEach((item: any) => {
        content += `${item.name},${item.sold}\n`;
      });
    } else if (type === 'inventory') {
      content = "Kode Barang,Nama Barang,Stok,Harga Beli\n";
      stats.gudangItems.forEach(item => {
        content += `${item.kode_brng},${item.nama_brng},${item.stok},${item.h_beli}\n`;
      });
    }
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <div className="flex gap-2 no-print">
                  <Button variant="outline" size="sm" onClick={() => handleDownload('kunjungan')}>
        ...
        <TabsContent value="resep" className="space-y-4">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <div className="flex gap-2 no-print">
                <Button variant="outline" size="sm" onClick={() => handleDownload('resep')}>
                  <FileText className="h-4 w-4" />
                </Button>
                <Button variant="outline" size="sm" onClick={handlePrint}>
                  <Printer className="h-4 w-4" />
